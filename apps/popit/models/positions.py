...
class Position(ModelBase):
    ...
    def __unicode__(self):
        if self.organisation:
            ...
            organisation = u'Unknown'
        if self.title and self.type:
            title = u'%s (%s)' % (self.title, self.type)
        elif self.type:
            title = self.type
        else:
            title = self.title or u'Unknown'
        if self.place:
            place = u'(%s)' % self.place
        else:
            place = ''
        name = str(self.person.name)
        out = u"%s's position as %s %s at %s (%s-%s)" % (self.person.name, title, self.place, organisation, self.start_date, self.end_date)
        return out
